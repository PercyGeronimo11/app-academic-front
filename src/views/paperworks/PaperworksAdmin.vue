                  ...
                  <CTableRow>
                    <CTableDataCell colspan="8" class="table-empty-cell">
                      <div class="table-empty-unified">
                        ...
                        <p class="table-empty-unified__title">No hay trámites pendientes</p>
...
const openObserve = async (item) => {
  const { value: text } = await Swal.fire({
    title: 'Observaciones de administración',
    input: 'textarea',
    inputLabel: 'Indique qué debe corregir el solicitante (volverá a mesa de partes)',
  ...
  if (!text) return;
  try {
    await PaperworkService.observeDirector(item.id, text.trim());
    await load();
    Swal.fire('Listo', 'Trámite observado; el solicitante deberá subsanar y volverá a mesa de partes.', 'success');
  } catch (e) {
